...
public class ShulkerInventory implements Inventory, NamedScreenHandlerFactory {
    ...
    public ShulkerInventory(ItemStack stack) {
        this.stack = stack;
    ...
    public void onOpen(PlayerEntity player) {
        if (!player.isSpectator()) {
            ...
            if (player instanceof ServerPlayerEntity serverPlayerEntity) {
                ...
                ServerPlayNetworking.send(serverPlayerEntity, ShulkerPackets.OPENING_BOX, buf);
                PlayerLookup.tracking(player)
                        .forEach(player1 -> ServerPlayNetworking.send(player1, ShulkerPackets.OPENING_BOX, buf));
            }
    ...
    public void onClose(PlayerEntity player) {
        ...
        if (!player.isSpectator()) {
            ...
            if (player instanceof ServerPlayerEntity serverPlayerEntity) {
                PacketByteBuf buf = new PacketByteBuf(Unpooled.buffer());
                buf.writeInt(player.getId());
                ServerPlayNetworking.send(serverPlayerEntity, ShulkerPackets.CLOSING_BOX, buf);
                PlayerLookup.tracking(player)
                        .forEach(player1 -> ServerPlayNetworking.send(player1, ShulkerPackets.CLOSING_BOX, buf));
            }
